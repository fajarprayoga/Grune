$(function () {
    ...
    $.ajax( {
        type:'GET',
        url: rootUrl + '/api/admin/company/getPrefectureTabular',
        data:'_token = <?php echo csrf_token() ?>',
        dataType: 'json',
        success :(prefectureList) => {
            // prefectureList = data;
            table(prefectureList)
        }
    })
    ...
    var formatActionField = function (row, cell, formatterParams) {
        return '<form id="form-delete-' + row.getData()['id'] + '" action="' + rootUrl + '/company-delete/" method="get">' +
               '<a class="btn btn-primary" href="' + rootUrl + '/company/' + row.getData()['id'] + '/edit' + '" title="Edit"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;' +
    ...
    var table = (prefectureList) => {
        $("#datalist").tabulator({
            ...
            columns: [
                ...
                {title: "Prefecture", field: "prefecture.display_name", width: 150, headerFilter:"select", headerFilterPlaceholder: " ",headerFilterParams :() => {
                    var options = {};
                    prefectureList.forEach(function(row){
                        options[row.display_name] = row.display_name;
                    });
                    return options;
                }},
...
function PageDataInfo(data){
    ...
    if(getDataCount > 0 ){
        $('#datalist-header').removeClass('invisible').css("visibility","initial");;
